...
object AnyLayout {
 def useAnyLayout(container: Container, alignmentX: Float, alignmentY: Float, size: Size, ifMissingConstraint: Component => Constraint) {
  ...
  val layout = new LayoutManager2 {
   ...
   def preferredLayoutSize(parent: Container) = new Dimension(size.width(),size.height())
   ...
   def getInfo(component: Component, isX: Boolean): LayoutInfo = {
    val context = new LayoutContext { def parentSize = if (isX) container.getWidth else container.getHeight
                                      def preferredSize = { val preferredSize = component.getPreferredSize
                                                               if (isX) preferredSize.width else preferredSize.height }
                                      def layoutInfoFor(component1: Component): LayoutInfo = getInfo(component1, isX) }
